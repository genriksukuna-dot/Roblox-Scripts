...
local CONFIG = {
    ...
    -- NEXUS Usage Leaderboard backend.
    -- Set this to the public HTTPS URL of the backend when it is deployed.
    LeaderboardApiUrl = "",
...
local LeaderboardArea = Instance.new("Frame")
LeaderboardArea.Position = UDim2.new(0, 210, 0, 0)
LeaderboardArea.Size = UDim2.new(1, -210, 1, 0)
LeaderboardArea.BackgroundTransparency = 1
LeaderboardArea.Visible = false
LeaderboardArea.ZIndex = 20
LeaderboardArea.Parent = Body

local LeaderboardScroll = Instance.new("ScrollingFrame")
LeaderboardScroll.Position = UDim2.new(0, 17, 0, 17)
LeaderboardScroll.Size = UDim2.new(1, -34, 1, -34)
...
LeaderboardScroll.ZIndex = 21
LeaderboardScroll.Parent = LeaderboardArea

local LeaderboardPadding = Instance.new("UIPadding")
LeaderboardPadding.PaddingBottom = UDim.new(0, 16)
LeaderboardPadding.Parent = LeaderboardScroll

local LeaderboardTitle = createText(LeaderboardScroll, "NEXUS LEADERBOARD", 22, COLORS.White, Enum.Font.GothamBold)
LeaderboardTitle.Position = UDim2.new(0, 8, 0, 8)
LeaderboardTitle.Size = UDim2.new(1, -16, 0, 30)
LeaderboardTitle.ZIndex = 22

local LeaderboardSubtitle = createText(
    LeaderboardScroll,
    "LONGEST ACTIVE USAGE",
    10,
    COLORS.Muted,
    Enum.Font.GothamBold
)
LeaderboardSubtitle.Position = UDim2.new(0, 9, 0, 40)
LeaderboardSubtitle.Size = UDim2.new(1, -18, 0, 18)
LeaderboardSubtitle.ZIndex = 22

local LeaderboardStatus = createText(
    LeaderboardScroll,
    "LOADING...",
    9,
    COLORS.Muted,
...
LeaderboardStatus.Position = UDim2.new(0, 9, 0, 61)
LeaderboardStatus.Size = UDim2.new(1, -18, 0, 18)
LeaderboardStatus.ZIndex = 22

local LeaderboardList = Instance.new("Frame")
LeaderboardList.Position = UDim2.new(0, 0, 0, 88)
LeaderboardList.Size = UDim2.new(1, 0, 0, 0)
LeaderboardList.BackgroundTransparency = 1
LeaderboardList.ZIndex = 22
LeaderboardList.Parent = LeaderboardScroll
...
LeaderboardList.AutomaticSize = Enum.AutomaticSize.Y
...
local FooterLeaderboard = Instance.new("TextButton")
FooterLeaderboard.Name = "FooterLeaderboard"
FooterLeaderboard.AnchorPoint = Vector2.new(0.5, 0.5)
FooterLeaderboard.Position = UDim2.new(0.72, 0, 0.5, 0)
FooterLeaderboard.Size = UDim2.fromOffset(126, 34)
FooterLeaderboard.BackgroundColor3 = Color3.fromRGB(20, 72, 95)
FooterLeaderboard.BorderSizePixel = 0
FooterLeaderboard.Text = "LEADERBOARD"
FooterLeaderboard.TextColor3 = Color3.fromRGB(170, 225, 245)
FooterLeaderboard.TextSize = 10
FooterLeaderboard.Font = Enum.Font.GothamBold
FooterLeaderboard.AutoButtonColor = false
FooterLeaderboard.ZIndex = 9
FooterLeaderboard.Parent = Footer
round(FooterLeaderboard, 9)
local FooterLeaderboardStroke = stroke(FooterLeaderboard, Color3.fromRGB(35, 160, 210), 1.4, 0.08)
...
FooterLeaderboard.MouseEnter:Connect(function()
    tween(FooterLeaderboard, { BackgroundColor3 = Color3.fromRGB(28, 105, 135) }, 0.12):Play()
    tween(FooterLeaderboardStroke, { Transparency = 0 }, 0.12):Play()
    tween(FooterLeaderboard, { TextColor3 = COLORS.White }, 0.12):Play()
end)

FooterLeaderboard.MouseLeave:Connect(function()
    tween(FooterLeaderboard, { BackgroundColor3 = Color3.fromRGB(20, 72, 95) }, 0.12):Play()
    tween(FooterLeaderboardStroke, { Transparency = 0.08 }, 0.12):Play()
    tween(FooterLeaderboard, { TextColor3 = Color3.fromRGB(170, 225, 245) }, 0.12):Play()
end)
...
-- Runs independently from the GUI so destroying the hub does
-- not stop the usage session.
--=======================================================

local usageSessionId = nil
local usageTrackingStarted = false
local usageTrackingStopped = false

local function getLeaderboardBaseUrl()
    local url = tostring(CONFIG.LeaderboardApiUrl or "")
    url = url:gsub("%s+$", "")
    url = url:gsub("/+$", "")
    return url
end
...
local function leaderboardRequest(method, path, body)
    local base = getLeaderboardBaseUrl()
    if base == "" then
    ...
    local okJson, encoded = pcall(function()
        return game:GetService("HttpService"):JSONEncode(body or {})
    end)
    if not okJson then
    ...
    if responseBody ~= "" then
        pcall(function()
            decoded = game:GetService("HttpService"):JSONDecode(responseBody)
        end)
    end
...
local function leaderboardIdentity()
    local avatarUrl = ""
    pcall(function()
...
local function startUsageTracking()
    if usageTrackingStarted or usageTrackingStopped then
        return false
    end
    ...
    task.spawn(function()
        local identity = leaderboardIdentity()
        local ok, response = leaderboardRequest("POST", "/api/session/start", identity)

        if not ok or type(response) ~= "table" or not response.sessionId then
            warn("[Nexus Leaderboard] Session start failed:", response)
            usageTrackingStopped = true
            return
        end

        usageSessionId = tostring(response.sessionId)

        while not usageTrackingStopped and usageSessionId do
            task.wait(15)
            if usageTrackingStopped then break end

            local heartbeatBody = leaderboardIdentity()
            heartbeatBody.sessionId = usageSessionId

            local hbOk, hbResponse = leaderboardRequest(
                "POST",
            ...
            if not hbOk then
                warn("[Nexus Leaderboard] Heartbeat failed:", hbResponse)
            elseif type(hbResponse) == "table" and hbResponse.reason == "invalid_session" then
                usageTrackingStopped = true
                usageSessionId = nil
            end
        end
...
local function stopUsageTracking()
    if usageTrackingStopped then return end
    usageTrackingStopped = true

    local sessionId = usageSessionId
    usageSessionId = nil

    if not sessionId then return end

    task.spawn(function()
        local body = leaderboardIdentity()
        body.sessionId = sessionId
        leaderboardRequest("POST", "/api/session/stop", body)
    end)
end

local function formatUsageTime(totalSeconds)
...
local function createLeaderboardRow(user, rank)
    local row = Instance.new("Frame")
    row.Size = UDim2.new(1, -4, 0, 64)
    row.BackgroundColor3 = Color3.fromRGB(17, 21, 31)
    row.BorderSizePixel = 0
    row.ZIndex = 23
    row.LayoutOrder = rank
    row.Parent = LeaderboardList
    round(row, 10)
    stroke(row, COLORS.Border, 1, 0.35)

    local rankText = createText(row, "#" .. tostring(rank), 9, COLORS.Muted, Enum.Font.GothamBold, Enum.TextXAlignment.Center)
    rankText.Size = UDim2.fromOffset(32, 64)
    rankText.Position = UDim2.fromOffset(2, 0)
    rankText.ZIndex = 24

    local avatar = Instance.new("ImageLabel")
    avatar.Size = UDim2.fromOffset(44, 44)
    avatar.Position = UDim2.new(0, 38, 0.5, -22)
    avatar.BackgroundColor3 = Color3.fromRGB(27, 31, 42)
    avatar.BorderSizePixel = 0
    avatar.ZIndex = 24
    avatar.Parent = row
    round(avatar, 10)
    stroke(avatar, Color3.fromRGB(100, 105, 125), 1, 0)

    pcall(function()
        avatar.Image = tostring(user.avatarUrl or "")
    end)

    local displayName = tostring(user.displayName or user.username or "Unknown")
    local username = tostring(user.username or "")

    local nameText = createText(row, displayName, 11, COLORS.White, Enum.Font.GothamBold)
    nameText.Position = UDim2.new(0, 92, 0, 9)
    nameText.Size = UDim2.new(1, -245, 0, 18)
    nameText.TextTruncate = Enum.TextTruncate.AtEnd
    nameText.ZIndex = 24

    local userText = createText(row, "@" .. username, 8, COLORS.Muted, Enum.Font.GothamMedium)
    userText.Position = UDim2.new(0, 92, 0, 31)
    userText.Size = UDim2.new(1, -245, 0, 15)
    userText.TextTruncate = Enum.TextTruncate.AtEnd
    userText.ZIndex = 24

    local timeText = createText(
        row,
        formatUsageTime(user.totalSeconds),
        10,
        Color3.fromRGB(170, 225, 245),
        Enum.Font.GothamBold,
        Enum.TextXAlignment.Right
    )
    timeText.Position = UDim2.new(1, -140, 0, 10)
    timeText.Size = UDim2.fromOffset(125, 18)
    timeText.ZIndex = 24

    local statusDot = Instance.new("Frame")
    statusDot.Size = UDim2.fromOffset(8, 8)
    statusDot.Position = UDim2.new(1, -20, 0, 39)
    statusDot.BackgroundColor3 = user.online and Color3.fromRGB(55, 214, 122) or Color3.fromRGB(100, 105, 115)
    ...
    local statusText = createText(
        row,
        user.online and "ONLINE" or "OFFLINE",
        7,
        user.online and Color3.fromRGB(75, 220, 135) or COLORS.Muted,
        Enum.Font.GothamBold,
        Enum.TextXAlignment.Right
    )
    statusText.Position = UDim2.new(1, -48, 0, 36)
    statusText.Size = UDim2.fromOffset(65, 14)
    statusText.ZIndex = 24
end

local function refreshLeaderboard()
    if not LeaderboardArea.Visible then return end

    local base = getLeaderboardBaseUrl()
    if base == "" then
        LeaderboardStatus.Text = "API URL NOT CONFIGURED"
        return
    end

    local req = getHttpRequest()
    if not req then
        LeaderboardStatus.Text = "HTTP REQUEST NOT AVAILABLE"
        return
    end

    local ok, response = leaderboardRequest("GET", "/api/leaderboard?limit=50", nil)
    if not ok or type(response) ~= "table" then
    ...
    local users = response.users or {}
    for index, user in ipairs(users) do
        createLeaderboardRow(user, index)
    end

    LeaderboardStatus.Text = tostring(#users) .. " USERS • LIVE"
end
    ...
    task.spawn(function()
        ...
        -- Start counting only after the game script has been downloaded and
        -- compiled, immediately before its actual execution.
        startUsageTracking()
...
local function selectCategory(category)
    ...
    local leaderboardActive = string.upper(category) == "LEADERBOARD"

    CardsArea.Visible = not creditsActive and not leaderboardActive
    CreditsArea.Visible = creditsActive
    LeaderboardArea.Visible = leaderboardActive
...
FooterLeaderboard.Activated:Connect(function() selectCategory("LEADERBOARD") end)
Search:GetPropertyChangedSignal("Text"):Connect(updateCardVisibility)

task.spawn(function()
    while true do
        task.wait(5)
        if LeaderboardArea.Visible then
