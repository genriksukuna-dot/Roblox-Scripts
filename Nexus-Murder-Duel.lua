...
local function makeTab(i, data)
    ...
    b.BorderSizePixel = 0
    b.Position = UDim2.fromOffset(7, 8 + ((i - 1) * 48))
...
makeTitle(SettingsPage, "SETTINGS", "Game data • interface • function state")
...
updateScale = function()
    ...
    if viewport.X < 600 then
        ...
        for _, ref in pairs(TabRefs) do
            ...
            ref.Icon.Size = UDim2.fromOffset(18, 40)
        ...
        for _, ref in pairs(TabRefs) do
            ...
            ref.Icon.Size = UDim2.fromOffset(22, 40)
...
local ESPInterval = 0.09
local nextESPUpdate = 0

local AimInterval = 0.045
local nextAimUpdate = 0

local UIInterval = 0.25
local nextUIUpdate = 0
local ESPVisualInterval = 0.05
local lastESPVisualUpdate = -math.huge
...
connect(RunService.RenderStepped, function()
    ...
    if aimPipelineActive and now >= nextAimUpdate then
        nextAimUpdate = now + AimInterval
    ...
    if now >= nextESPUpdate then
        nextESPUpdate = now + ESPInterval
    ...
    if now >= nextUIUpdate then
        nextUIUpdate = now + UIInterval

        dFPS.Text = tostring(fpsValue)
...
RunService:BindToRenderStep(
    ...
    function()
        ...
        if now - lastESPVisualUpdate < ESPVisualInterval then
            return
        end
        lastESPVisualUpdate = now
...
switchTab("ESP")
print("[Nexus Murder Duel] Stable ESP + hitbox + follow teleport + native tool shot + lobby protection loaded.")
