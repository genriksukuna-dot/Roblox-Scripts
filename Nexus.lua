...
--//  • Search + ALL / SHUTER filtering retained
...
local function setImage(imageLabel, assetId)
    ...
    -- Try several Roblox content URI forms. Some image/decal assets resolve
    -- through the thumbnail service while others resolve directly.
    local uris = {
        "rbxassetid://" .. id,
    ...
    task.spawn(function()
        for _, uri in ipairs(uris) do
            ...
            -- Give Roblox's image pipeline a little extra time on mobile.
...
-- Two green dots + Online, moved left so the controls don't overlap.
...
local AllButton = createSidebarButton("All", "ALL", 62)
local ShuterButton = createSidebarButton("Shuter", "SHUTER", 110)
...
local function closeHubForLaunch()
    ...
    -- Stop interaction while the hub folds away.
    Main.Active = false
    Body.Active = false
    DetailsPage.Active = false

    -- A small perspective-style fold: zoom, tilt, lift and fade the backdrop.
    -- Roblox GUI rotation is 2D, so the 3D feeling is simulated with scale,
    -- rotation, movement and an overlaid depth/shadow layer.
...
-- SCRIPT EXECUTION
--=======================================================
-- Both LOAD SCRIPT and RUN intentionally use the exact same
-- selected-game script URL. This keeps the two buttons tied
...
local function launchSelected(cfg)
    ...
    -- Capture the selected game before the UI is destroyed.
    local selected = cfg
...
-- CLOSE BUTTON: connected independently so a later runtime error can't kill it.
