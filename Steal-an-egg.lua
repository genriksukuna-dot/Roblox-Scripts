...
warn("[STEAL EGG PRO] ОШИБКА: Не найдено место для GUI!")
return
end
​pcall(function()
...
Title.Text = "STEAL AN EGG | PRO V9"
Title.TextSize = 11.5
...
​createToggle(mainPage, "PHYSICS SPEED BYPASS", function(state)
Config.SpeedEnabled = state
end)
​createSlider(mainPage, "WALK SPEED LIMIT", 16, 110, 110, function(val)
...
if name:find("fuse") or name:find("machine") or name:find("cube") or name:find("mesh") or name:find("ui") then return false end
...
​print("[STEAL EGG PRO] === УСПЕШНО ЗАГРУЖЕНО ===")
end)
​if not success then
warn("[STEAL EGG PRO КРИТИЧЕСКАЯ ОШИБКА]: " .. tostring(err))
end
